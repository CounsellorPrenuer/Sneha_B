...
const getData = async () => {
  ...
  return {
    homepage:
      ...
      (founder
        ? {
            heroTitle: founder.title ? `${founder.title}: ${founder.name || "MindAxis"}` : founder.name || "MindAxis",
            heroSubtitle:
              typeof founder.bio === "string"
                ? founder.bio.split("\n")[0]
                : "Scientific tool-based career guidance powered by AI.",
...
export default async function Home() {
  ...
  const missionParagraphs = portableTextToParagraphs(mission?.content);
